...
import {SocketUser, UserOnlineProps} from "../../interfaces/Chat";
...
const ChatOnline: FC<UserOnlineProps> = ({onlineUsers, setCurrentChat, currentUserId}) => {
    ...
    const handleChangeConversation = async (user: User) => {
        try {
            if (currentUserId) {
                const data = await Api.conversation.getCurrentUserConversation(
                    currentUserId,
                    user._id
                );
                setCurrentChat(data);
    ...
    return (
        <div className={style.chatOnline}>
            {onlineFriends?.map((friend, i) => (
                <div
                    key={i}
                    className={style.chatOnlineFriend}
                    onClick={() => handleChangeConversation(friend)}
                >
